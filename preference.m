...
function reward = preference(xi, xj, delta, deltarange,pref_mode)
    ...
    elseif pref_mode == "normdif"    % yet another alternative function, but non-linear
        difference = abs(d - delta);
        normed_difference = difference ./ deltarange;
        reward = (1 - normed_difference) ./ (1 + normed_difference);
    elseif pref_mode == "abs"    % alternative (linear) preference function       
        difference = abs(d - delta);
